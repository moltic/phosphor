 ...
 * @param {string} html – innerHTML produced by buildBannerHtml(), never raw user text
 */
export function printBannerHtml(html) {
  const pre = document.createElement('pre');
  pre.className = 'banner-output';
  pre.innerHTML = html; // safe — generated by buildBannerHtml
  if (_batchEl) {
...
/** Build the neon banner HTML from a raw multi-line ASCII-art string. */
export function buildBannerHtml(raw, { preserveGlyphs = true } = {}) {
...
/** Render banner text into an HTML grid for command output. */
...
/** Set the #ascii-art element's content (HTML or plain text). */
export function setAsciiArt(el, text, { asHtml = true } = {}) {
  const val = text || '';
